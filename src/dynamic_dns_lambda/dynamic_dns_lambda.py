...
config_s3_bucket = 'lambdadyndnsconfig'
...
def run_set_mode(set_hostname, validation_hash, source_ip):
    ...
    calculated_hash = hashlib.sha256(
        source_ip.encode() + set_hostname.encode() + shared_secret.encode()).hexdigest()
    ...
    # the hostname DNS record from Route 53.
        ...
        if route53_ip == source_ip:
            return_status = 'success'
            return_message = 'Your IP (' + source_ip + ') address matches '\
